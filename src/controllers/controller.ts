...
const root = async (request: Request, response: Response) => {
...
const addDog = async (
  ...
  let statementResult = dataManager.run(
    "INSERT INTO `dogs` (`name`, `age`, `breed`, `gender`) VALUES (?, ?, ?, ?)",
    [
      request.body.name,
      request.body.age,
      request.body.breed,
...
const updateDog = async (
  ...
  dataManager.run(
    "UPDATE `dogs` SET `name` = ?, `age` = ?, `breed` = ?, `gender` = ? WHERE `id` = ?",
    [
      request.body.name,
      request.body.age,
      request.body.breed,
...
export default { root, addDog, updateDog, getDogs, getDog, deleteDog };
